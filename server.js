...
app.get("/admin-gdpr-search/:clientId", adminLimiter, async (req, res) => {
  try {
    const clientId = sanitizeClientId(req.params.clientId);
    if (!clientId) return res.status(400).json({ error: "Neispravan zahtjev." });
...
app.get("/admin-gdpr-export/:clientId", adminLimiter, async (req, res) => {
  try {
    const clientId = sanitizeClientId(req.params.clientId);
    if (!clientId) return res.status(400).json({ error: "Neispravan zahtjev." });
...
app.delete("/admin-gdpr-delete/:clientId", adminLimiter, async (req, res) => {
  try {
    const clientId = sanitizeClientId(req.params.clientId);
    if (!clientId) return res.status(400).json({ error: "Neispravan zahtjev." });
    if (!gdprAuth(req, res, clientId)) return;
...
cron.schedule("0 3 1 * *", async () => {
  ...
  const granicaId = granica.getTime();
